...
- **`passes/pass5_meaning.py` → thin LOCAL-FACT meaning, AFTER chunk entities.** Rewrite `_SYSTEM` (lines 16-21) to **forbid the meta-phrase family** ("this chunk indicates/shows/reflects/documents…", "the significance to the claim/denial is…", "this is important because…") **and whole-claim significance**, and to **demand a small standalone concrete fact whose nouns are the chunk's resolved entities — dated WHEN the chunk's own text carries a date** (a dated chunk → "On 2025-06-06, an extension request was submitted for case 00716392."; the gold note, whose text has **no** date → "Manager B. Smith placed the claim back under the Mental Health limitation." — no date asserted, since none is grounded in the span). **Drop `questions_answered`** (delete from `_SYSTEM`, `parse_meaning`, and the returned dict). Meaning is GROUNDED at `Tier.INTERPRETATION` via the **new `MeaningWriter`** (§5.4) and persisted to `enrichment_meaning` — **not** through `WriteEnrichment`. `per_chunk` stays `True`; it runs AFTER `EntityNormalizationPass` so resolved entities are in `prior`.
...
- **(d) Name the gold-note REQUIRED EDGES** in the agentic-walk acceptance (§10 AGENTIC): the reversal record (`89503c71…`) must link to the decision records it undid and to the `B. Smith` actor node; and — since its own date is metadata, not in-text — to the reversal's **date node sourced from the note metadata or a linked neighbor** (value per the source; **do not assume `2025-02-18`** — the corpus check suggests it may be `2024-02-18`/bare `Feb 18`).
...
- **GOLD-NOTE FLOOR FIRST** (hard regression gate, runs before anything else): chunk `89503c71-5ca2-424b-9386-6698a8337dc3` yields **≥1 record GROUNDED ON ITS OWN TEXT naming `B. Smith` AND the reversal content** (`Mental Health limitation` + "place claim back"). This is the audition-proven capture — both candidates grounded exactly this, and the record's `date` was `None`. **The gold note's text carries NO date**; the Feb reversal date is the note's *metadata*, not its body, so it is **NOT part of the hard floor** (requiring it grounded-on-chunk would fail spuriously). If the B. Smith + reversal grounding fails, **stop**.
- THEN: actor `B. Smith` and the event "back-to-MH-limitation" (both grounded on the chunk) **EACH resolve to exactly ONE node.** The reversal's date — captured as a metadata/linked entity (§8d), value taken from the note's source record (**not assumed**) — resolves to one node *if present*; this is a **soft** check, not the hard floor.
